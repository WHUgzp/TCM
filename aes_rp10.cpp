#include "aes_rp10.h"
#include "aes.h"
#include "share.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

byte sq_1[256] = {
	 };

byte taffine_1[256] = {
	 };

byte tsmult_1[1024] = {
	 };

byte square_1(byte x)
{
	//return mult(x,x);
	return sq_1[x];
}

// Computes z=x*y in GF(2^8) using four 8-bit tables 
byte multtable_1(byte x, byte y)
{
	return tsmult_1[0 | ((x & 15) << 4) | (y & 15)] ^
		tsmult_1[512 | (x & 240) | (y & 15)] ^
		tsmult_1[256 | ((x & 15) << 4) | (y >> 4)] ^
		tsmult_1[768 | (x & 240) | (y >> 4)];
}

// AES Sbox computation without masking
byte subbyte_rp10(byte x)
{
	byte u2 = square_1(x);
	byte u3 = multtable_1(u2, x);
	byte u6 = square_1(u3);
	byte u7 = multtable_1(u6, x);
	byte u14 = square_1(u7);
	byte u15 = multtable_1(u14, x);
	byte u240 = square_1(square_1(square_1(square_1(u15))));
	byte u254 = multtable_1(u240, u14);
	return taffine_1[u254];
}

void square_share_1(byte *a, int n)
{
	int i;
	for (i = 0; i<n; i++)
		a[i] = square_1(a[i]);
}

// The shared multiplication SecMult of Rivain-Prouff
void multshare_1(byte *a, byte *b, byte *c, int n)
{
	// Memory: 4 bytes
	int i, j;
	for (i = 0; i<n; i++)
		c[i] = multtable_1(a[i], b[i]);

	for (i = 0; i<n; i++)
	{
		for (j = i + 1; j<n; j++)
		{
			byte tmp = xorshf96(); //rand();
			byte tmp2 = (tmp ^ multtable_1(a[i], b[j])) ^ multtable_1(a[j], b[i]);
			c[i] ^= tmp;
			c[j] ^= tmp2;
		}
	}
}

void subbyte_rp10_share(byte *a, int n)
{
	// Memory: 5*n+5 byte 
	int i;
	byte z[20];
	memcpy(z, a, n);
	square_share_1(z, n);    // z=x^2     

	byte y[20];
	multshare_1(z, a, y, n);   // y=z*x=x^3

	byte w[20];
	memcpy(w, y, n);
	square_share_1(w, n);
	square_share_1(w, n);     // w=x^12

	byte y2[20];
	multshare_1(y, w, y2, n);   // y2=x^15

	square_share_1(y2, n);
	square_share_1(y2, n);
	square_share_1(y2, n);
	square_share_1(y2, n);    // y2=x^240

	multshare_1(w, y2, y, n);   // y=x^252
	multshare_1(y, z, a, n);    // a=x^254

	for (i = 0; i<n; i++)
		a[i] = taffine_1[a[i]];
	if ((n % 2) == 0)
		a[0] ^= 99;
}

void subbytestate_rp10(byte *state)
{
	int i;
	for (i = 0; i<16; i++) state[i] = subbyte_rp10(state[i]);
}


void aes_rp10(byte in[16], byte out[16], byte key[16])
{
	int i, j;
	int round = 0;
	byte state[16];
	byte w[176];

	keyexpansion(key, w);

	for (i = 0; i<16; i++)
		state[i] = in[i];

	addroundkey(state, w, 0);

	for (round = 1; round<10; round++)
	{
		subbytestate_rp10(state);
		shiftrows(state);
		mixcolumns(state);
		addroundkey(state, w, round);
	}

	subbytestate(state);
	shiftrows(state);
	addroundkey(state, w, 10);

	for (i = 0; i<16; i++)
		out[i] = state[i];
}


