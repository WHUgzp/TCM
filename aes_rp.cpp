	...
	else 
	{
		...
		for (i = 0; i < n; i++)
		{
			for (j = 0; j < n; j++)
			{
				...
				{
					M_22[i - half][j - half] = M[i][j];
				}
			}
		}

		matMult(a_1, b_1, M_11, half);
		refresh(a_1, half);
		refresh(b_1, half);
		matMult(a_1, b_2, M_12, half); 
		matMult(a_2, b_1, M_21, half);
		refresh(a_2, n - half);
		refresh(b_2, n - half);
		matMult(a_2, b_2, M_22, half);
